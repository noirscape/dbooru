    ...
    class Extractor
        class Kemono < Source::Extractor
            def image_urls
                ...
                paths = []
                paths << api_post_response.dig("file", "path")
                api_post_response["attachments"].each do |attachment|
            ...
            def artist_commentary_title
                api_post_response.dig("title")
            end

            def artist_commentary_desc
                api_post_response.dig("content")
            end
            ...
            def api_post_response
                api_response.dig("post")
            end

            memoize def artist_response
                http.cache(1.minute).parsed_get("https://kemono.su/api/v1/creators.txt", format: :json) || {}
            end
